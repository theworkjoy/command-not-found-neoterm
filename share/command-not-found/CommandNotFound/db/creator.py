...
class DbCreator:
    ...
    def _fill_commands(self, con):
        for f in self.files:
            with subprocess.Popen(["/data/data/io.neoterm/files/usr/lib/apt/apt-helper", "cat-file", f], stdout=subprocess.PIPE) as sub:
                if "Contents" in f:
                ...
                if sub.wait() != 0:
                    raise subprocess.CalledProcessError(returncode=sub.returncode,
                                                         cmd="/data/data/io.neoterm/files/usr/lib/apt/apt-helper cat-file {}".format(f))
    ...
    def _parse_single_contents_file(self, con, f, fp):
        ...
        for l in fp:
            l = l.decode("utf-8")
            if not (l.startswith('data/data/io.neoterm/files/usr/sbin') or l.startswith('data/data/io.neoterm/files/usr/bin') or
                    l.startswith('data/data/io.neoterm/files/usr/bin') or l.startswith('data/data/io.neoterm/files/usr/sbin')):
                continue
            try:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        print(" e.g.: %s /data/data/io.neoterm/files/usr/var/lib/command-not-found/commands.db  /data/data/io.neoterm/files/usr/var/lib/apt/lists/*Commands-*", sys.argv[0])
        sys.exit(1)
